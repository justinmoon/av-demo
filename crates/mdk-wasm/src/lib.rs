...
#[cfg(not(feature = "with-mdk"))]
fn feature_disabled() -> JsValue {
    js_error("with-mdk feature disabled")
}
...
pub fn create_identity(secret_hex: String) -> Result<u32, JsValue> {
    ...
    #[cfg(feature = "with-mdk")]
    {
        let secret = SecretKey::from_hex(&secret_hex)
            .map_err(|e| js_error(format!("invalid secret: {e}")))?;
...
pub fn create_group(
    ...
    #[cfg(not(feature = "with-mdk"))]
    {
        let _ = (identity_id, config, member_events);
        return Err(feature_disabled());
    }
...
pub fn accept_welcome(identity_id: u32, welcome_json: String) -> Result<JsValue, JsValue> {
    #[cfg(not(feature = "with-mdk"))]
    {
        let _ = (identity_id, welcome_json);
        return Err(feature_disabled());
    }
...
pub fn self_update(identity_id: u32, group_id_hex: String) -> Result<JsValue, JsValue> {
    #[cfg(not(feature = "with-mdk"))]
    {
        let _ = (identity_id, group_id_hex);
        return Err(feature_disabled());
    }
